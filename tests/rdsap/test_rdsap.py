...
local = True
url_sap = 'http://127.0.0.1:8000/calc/sap10' if local else None
url_rdsap = 'http://127.0.0.1:8000/calc/rdsap10' if local else None
auth_token = None
...
def run_test(
    ...
    ""
    # save input XML
    fp_in = f'{name}_in.xml'
    tree.write(fp_in, encoding="utf-8", xml_declaration=True, pretty_print=True)

    # save input XML - version display
    fp_out = f'{name}_in__display.xml'
    with open(fp_out, 'w') as f: 
        f.write(tree.getroot().display())  

    # run RdSAP engine
    result = sap10calcs.rdsap(input_file = fp_in, url = url_rdsap, auth_token = auth_token)

    # save files, print error message if present
    fp_out = f'{name}_out_rdsap.json'
    with open(fp_out, 'w') as f: json.dump(result, f, indent = 4)  
    if not result['rdsap_calculation_error_traceback'] is None:   
        print('\n', result['rdsap_calculation_error_traceback'])
    else:
        fp_out = f'{name}_out_rdsap.xml'
        tree, sap_report = sap10calcs.parse_xml(StringIO(result['sap_xml']))  
        tree.write(fp_out, encoding="utf-8", xml_declaration=True, pretty_print=True)
        fp_out = f'{name}_out_rdsap__display.xml'
        with open(fp_out, 'w') as f: 
            f.write(sap_report.display())  

    # run SAP engine
    fp_in = f'{name}_out_rdsap.xml'
    result = sap10calcs.calculate(input_file = fp_in, url = url_sap, auth_token = auth_token)
            
    # save files, print error message if present
    fp_out = f'{name}_out_sap_energy_rating.json'
    with open(fp_out, 'w') as f: json.dump(result, f, indent = 4)  
    if not result['sap_calculation_error_traceback'] is None:   
        print('\n', result['sap_calculation_error_traceback'])
